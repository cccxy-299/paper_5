...
from tad.models.sparse_model import SparseConvNeXtV2
from tad.models.conv_model import Block
...
class FCMAE(nn.Module):
    ...
    def forward_decoder(self, x, mask):
        ...
        # pred
        pred = self.pred(x)
        # 调整pred尺寸以适应unpatchify (N, L, patch_size**2 * 3)
        pred = pred.reshape(n, self.patch_size**2 * 3, -1).permute(0, 2, 1)
    ...
    def forward_loss(self, imgs, pred, mask):
        """
        imgs: [N, 3, H, W]
        pred: [N, 3, H, W] 重建后的完整图像
        mask: [N, L], 0表示保留，1表示移除
        """
        # 先将pred和imgs转换为patches
        target = self.patchify(imgs)  # (N, L, p*p*3)
        pred = self.patchify(pred)  # (N, L, p*p*3)
        ...
        loss = (pred - target) ** 2
        loss = loss.mean(dim=-1)  # 每个patch的平均loss (N, L)

        loss = (loss * mask).sum() / mask.sum()  # 仅对被mask掉的区域计算损失
        return loss
    '''最初'''
    # def forward_loss(self, imgs, pred, mask):
    #     """
    ...
    def forward(self, imgs, labels=None, mask_ratio=0.6,only_encoder=False):
        x, mask = self.forward_encoder(imgs, mask_ratio)
        if only_encoder:
            x = self.pooling(x)
            x = x.squeeze(-1).squeeze(-1)  # 结果：形状是 (B, 512)
            return x
